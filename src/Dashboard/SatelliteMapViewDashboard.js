// Move the existing SatelliteMapView.js content here without changes
// ...existing SatelliteMapView code...
import React, { useEffect, useState } from 'react';
import { MapContainer, TileLayer, Circle, Polyline, Marker, Popup, Tooltip } from 'react-leaflet';
import L from 'leaflet';
import 'leaflet/dist/leaflet.css';

// Update the satellite icon to include ID
const createSatelliteIcon = (id) => L.divIcon({
  html: `<div class="satellite-marker">🛰️<span class="satellite-label">${id}</span></div>`,
  className: 'satellite-icon-container',
  iconSize: [40, 40],
  iconAnchor: [20, 20],
});
 
// Update the antenna icon creation
const antennaIcon = L.divIcon({
  html: '<div class="antenna-marker">📡</div>',
  className: 'antenna-icon-container',
  iconSize: [24, 24],
  iconAnchor: [12, 12],
});
...
const SatelliteMapViewDashboard = ({ onRangeUpdate, focusSatelliteId = '1A2331' }) => {  // Changed from SatelliteMapView to SatelliteMapViewDashboard
  ...
  // Add new state for map view
  const [mapView, setMapView] = useState(null);

  // Add new reference to track the focused satellite
  const [focusedSat, setFocusedSat] = useState(null);
  ...
  // Add effect to update map view when focused satellite position changes
  useEffect(() => {
    if (focusSatelliteId) {
      const focusedSatellite = satellitePositions.find(sat => sat.id === focusSatelliteId);
      if (focusedSatellite && mapView) {
        mapView.setView(focusedSatellite.position, 4);
      }
    }
  }, [satellitePositions, focusSatelliteId, mapView]);

  // Modified useEffect to track satellite position
  useEffect(() => {
    if (mapView && satellitePositions.length > 0) {
      const targetSat = satellitePositions.find(sat => sat.id === '1A2331');
      if (targetSat) {
        mapView.setView(targetSat.position, 4, {
          animate: true,
          duration: 1
        });
        setFocusedSat(targetSat);
      }
    }
  }, [mapView, satellitePositions]);
  ...
  return (
    ...
    >
      ...
      {globalAntennas.map((antenna, index) => (
        <React.Fragment key={index}>
          <Marker
            ...
            opacity={focusedSat?.connectedAntennas.some(a => a.name === antenna.name) ? 1 : 0.4}
          >
            <Popup>
              <div>
                <h3>{antenna.name}</h3>
          ...
          <Circle
            ...
            pathOptions={{
              ...
              fillOpacity: focusedSat?.connectedAntennas.some(a => a.name === antenna.name) ? 0.2 : 0.05,
              weight: focusedSat?.connectedAntennas.some(a => a.name === antenna.name) ? 2 : 1
            }}
          />
      ...
      {/* Add highlighted path for focused satellite */}
      {orbitPaths
        .filter(orbit => orbit.id === focusSatelliteId)
        .map((sat) =>
      ...
      {/* Update satellite markers to highlight focused satellite */}
      {satellitePositions.map((sat, index) => (
        <Marker
          key={index}
          position={sat.position}
          icon={createSatelliteIcon(sat.id)}
          zIndexOffset={sat.id === focusSatelliteId ? 3000 : 2000}
          opacity={sat.id === focusSatelliteId ? 1 : 0.5}
        >
